...
- **Cleanup**: `server/scripts/cleanup-old-posts.js` is manual only (`--confirm` to delete); it is **not** wired to cron. Deletes non-featured **cron** posts (`source = 'cron'`) older than 15 days; manual posts are never targeted.
...
- **DB**: Run `server/migrations/add-tetris-scores.sql` once on Neon (SQL Editor or `psql`). Requires `DATABASE_URL` on Vercel for `GET/POST /api/tetris/scores`.
- **Client**: `/tetris` posts scores on game over; personal best stays in `localStorage` (`tetris-high-score`).
