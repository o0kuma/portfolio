...
// Simple rate limit: 30 req/hour per IP
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json({ reply: 'AI 기능이 아직 설정되지 않았어요. Contact 페이지로 직접 연락해주세요!' })
    }
    ...
    const reply = res.content[0].type === 'text' ? res.content[0].text : '응답을 받지 못했어요.'
