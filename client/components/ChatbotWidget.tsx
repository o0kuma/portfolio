...
export default function ChatbotWidget() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'assistant', content: '안녕하세요! 포트폴리오에 대해 궁금한 것이 있으시면 무엇이든 물어보세요 😊' },
  ])
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (res.ok && data.reply) {
        ...
        setMessages((prev) => [
          ...prev,
          { role: 'assistant', content: data.message ?? '오류가 발생했습니다. 잠시 후 다시 시도해주세요.' },
        ])
      }
    } catch {
      setMessages((prev) => [
        ...
        <div className="w-80 h-[480px] flex flex-col rounded-2xl border border-neutral-700 bg-neutral-900 shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-neutral-800 border-b border-neutral-700">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-neutral-100">포트폴리오 어시스턴트</span>
            </div>
            <button
